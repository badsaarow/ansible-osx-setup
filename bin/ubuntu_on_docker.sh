#!/bin/sh
apt -y install curl vim sudo
adduser --disabled-password --gecos '' sungyong
usermod -aG docker,adm,dialout,cdrom,floppy,sudo,audio,dip,video,plugdev,netdev,lxd sungyong
su sungyong -c "cd ~;mkdir -p ~/.ssh;echo \"ssh-rsa  sungyong@LAPTOP-UN28N1E0\" | tee >> ~/.ssh/authorized_keys;chmod 600 ~/.ssh/*"
passwd sungyong
